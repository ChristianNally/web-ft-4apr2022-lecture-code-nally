...
COPY public.objectives (id, day_id, type, question, answer, sort, total_time, parent_id, curriculum_id) FROM stdin;
5	22	learning	How do I establish a relationship between tables?	You can establish a relationship between tables by creating a column that tells you, for each row, which row in another table it is related to.	5	\N	\N	1
27	22	learning	What are the features required of a primary key?	A Primary Key must be unique (within the table) and can never be null.	6	\N	\N	1
28	22	learning	What is the most common data type for a Primary Key?	A Primary Key's data type is usually auto-incrementing integer (INTEGER or BIGINT).	7	\N	\N	1
29	22	learning	What is a Foreign Key?	A Foreign Key is a column on a second table, where the value in a given row indicates an association with a row from the first table. The Foreign Key value on the second table matches the Primary Key value from the first table.	8	\N	\N	1
32	22	learning	What is an ERD?	ERD; an Entity Relationship Diagram is a diagram that shows the relationships established between entities within a system. For databases, this is more often with regard to relationships between tables.	0	\N	\N	1
13	16	learning	What is the box model?	The box model is how DOM elements are typically rendered to occupy a certain amount of screen real estate, via padding, borders and margins.	4	\N	\N	1
18	\N	learning	What is DOM traversal?	DOM traversal is the ability of client-side javascript to jump from one node to a parent, sibling or child, step by step throughout the DOM, looking for elements that match a certain criteria.	\N	\N	\N	1
36	23	performance	How do we prevent a SQL Injection attack?	client.query('SELECT * FROM <table> WHERE id = $1', [<id>])   \n.then((result) => console.log(result));	4	\N	\N	1
35	23	learning	What is a SQL injection attack?	A malicious user crafts input for a form submission that will be mistakenly input directly into a database. That input might be any SQL query, including dropping tables or deleting rows. 	3	\N	\N	1
34	23	performance	How, given a postgres client object, do you submit queries to the database from back-end javascript?	client.query('SELECT * FROM <table>')\n   .then((result) => console.log(result));	2	\N	\N	1
40	12	learning	In the context of web programming, what is a template?	A template is a file containing mainly HTML, but with syntactic morsels of dynamic content.	3	\N	\N	1
20	17	performance	How do you traverse the DOM with jQuery?	$('.parent').children().addClass("tagged");	4	\N	\N	1
2	21	learning	What is a database?	A database is a collection of tables. The collection is typically used as a set. A connection to a database is granted via a username and password.	3	\N	\N	1
4	21	learning	What is a WHERE clause?	A query can filter or restrict the information that results via a WHERE clause.	4	\N	\N	1
6	21	learning	What is a JOIN clause?	The data that a query has access to, can be expanded via JOIN-ing two tables together.	6	\N	\N	1
3	21	learning	What is a query?	A query is a task executed against a database, table or combination of tables. e.g. SELECT, INSERT, etc.	5	\N	\N	1
9	21	performance	How do I INSERT rows into a table?	INSERT INTO objectives(id, type, question, answer, sort)\\nVALUES (21, w05d1, "How do I INSERT rows into a table?", "solution goes here",2);	9	\N	\N	1
31	21	learning	What are the four types of commands for databases?	DDL, DML, DCL, and TCL. See: https://stackoverflow.com/questions/2578194/what-are-ddl-and-dml	8	\N	\N	1
7	21	learning	What is an ERD?	In the context of databases, an ERD is a diagram that shows each table as an entity, and also shows the relationships between tables.	7	\N	\N	1
1	21	learning	What is a table?	A table is a set of data collected as columns and rows.	1	\N	\N	1
30	21	learning	What are the queries that correspond to the CRUD actions?	The CRUD queries are: INSERT, SELECT, UPDATE and DELETE.	10	\N	\N	1
8	21	performance	How do I create a table?	CREATE TABLE objectives\\n(\\n    id bigint,\\n    day_id character varying(5),\\n    type character varying(12),\\n    question text,\\n    answer text,\\n    sort smallint\\n)	2	\N	\N	1
41	12	learning	What is the main reason to use a templating system?	Separating business logic from the presentation layer (separation of concerns), enables specialization of roles for programmers.	4	\N	\N	1
23	19	performance	How do you set a maximum width on HTML elements?	.these_elements { max-width: 135px; }	4	\N	\N	1
25	19	learning	What are the absolute length units available in CSS?	mm, cm, in, px, pt, pc (1 pc = 12 pt)	5	\N	\N	1
22	19	performance	How do you set a minimum width on HTML elements?	.these_elements { min-width: 135px; }	3	\N	\N	1
26	19	learning	What are the relative length units available in CSS?	em, ex, ch, rem, vw, vh, vmin, vmax, %	6	\N	\N	1
24	19	performance	How should we set the web browser's viewport?	<meta name="viewport" content="width=device-width, initial-scale=1.0">	2	\N	\N	1
10	14	learning	Why would we never want to store passwords as plaintext?	To keep the passwords away from prying eyes, like hackers and website employees.	2	\N	\N	1
39	18	learning	What are some disadvantages of AJAX?	Creating dynamic content is tricky, asynchronous programming patterns are more complex to code, it requires js and XMLHTTPRequest support, history is not automatically updated.	3	\N	\N	1
11	14	learning	Rather than storing passwords as plaintext, how should they be stored?	Passwords should always be _hashed_. There is no need to encrypt them, since you never want to be able to see the password yourself.	1	\N	\N	1
...
45	12	learning	Does the order that your routes appear in your source code matter?	Yes. The order of the routes in your file matters. First matched -> First used.	7	\N	\N	1
...
15	16	learning	What is the basic syntax for a CSS 'Rule'?	[selector] { style: value; ... } 	3	120	\N	1
14	16	learning	How do you set the preferred box model?	* { box-sizing: border-box; }	5	120	\N	1
...
60	16	learning	What are the three CSS styles most directly related to the box model for any given DOM element?	Margin, Border, Padding	6	\N	\N	1
61	16	learning	For the CSS styles that take 1,2 or 4 length parameters (padding, margin, etc.) how is each number of parameters interpreted?	For 1 parameter, all 4 sides of the box get the same value. For 2 parameters, the first is 'top' and 'bottom' and the second is 'left' and 'right'. Four parameters specify each side, top, right, bottom and left, respectively.	7	\N	\N	1
59	16	learning	What is a Single Page App?	An SPA is an app that leverages AJAX to prevent page reloads. Instead it changes page content by manipulating the DOM directly, using front-end javascript.	0	\N	\N	1
33	23	performance	How do you connect to a Postgres DB from within Javascript?	const pg = require('pg');\nconst config = { \n  user: '<user name>',\n  password: '<password>',\n  database: '<db>',\n  host: '<host>' };\nconst client = new pg.Client(config);\nclient.connect()\n.then(() => console.log('db connected'))\n.catch(err => console.error('db connection error', err.stack));	1	\N	\N	1
64	17	learning	What are some examples of Javascript objects that are only defined in the browser context?	window, navigator, document	0	\N	\N	1
16	17	performance	How do you create a new element using jQuery and dynamically append it to a DOM element?	$('<div class="newElement">Text content goes here.</div>').appendTo($( ".container" ));	2	\N	\N	1
65	17	learning	What is DOM traversal?	DOM travsersal is the ability for client-side Javascript to move from one node of the DOM to another. Test.	3	\N	\N	1
78	31	learning	What is React's Virtual DOM?	The virtual DOM is a copy of the DOM that it kept in memory. React does most of its work there, and then as a final step patches the actual DOM, but only where needed.	0	\N	\N	1
79	31	performance	How do you create a boilerplate React app?	On the terminal:\n\n$> npx create-react-app name-of-app-goes-here\n	1	\N	\N	1
70	19	learning	How do you define variables in SASS?	$main-size: 2em;\n$background-color: purple;	7	\N	\N	1
81	31	performance	How do you make your own HTML Tags using JSX?	In JSX, a new component can be made by creating a function that returns HTML.\n\nfunction Button(props){\n  return (\n    <button>{props.text}</button>\n  );\n}\n\nThe following two ways of executing that function from within HTML are equivalent:\n\n<Button text="click me"></Button>\n\nor\n\n{Button({text: 'click me'});	5	\N	\N	1
17	17	learning	What is DOM manipulation?	DOM manipulation is the ability of client-side javascript to add, remove or change parts of the DOM after the initial page load.	1	\N	\N	1
57	14	learning	How can you use verbs other than GET and POST in your HTML, given that you're using a NodeJS back-end?	The method-override package: https://www.npmjs.com/package/method-override	5	\N	\N	1
58	14	learning	What package is available to help you implemented encrypted cookie values?	cookie-session: http://expressjs.com/en/resources/middleware/cookie-session.html	6	\N	\N	1
37	18	learning	What is AJAX?	Asynchronous Javascript and XML is a different paradigm of web request, where smaller amounts of data, or small parts of a webpage are sent and recieved in requests that do not result in page a refresh.	0	\N	\N	1
56	14	learning	What is REST?	REST is a convention for choosing paths and HTTP verbs for end-point routes.	4	\N	\N	1
66	18	performance	How do you invoke an AJAX request from jQuery?	$.ajax({\n  url: url,\n  method: "GET", \n})\n.then((result) => {\n  console.log('result:',result);\n})\n.catch(err => {\n  console.log(err); // related error\n});	1	\N	\N	1
38	18	learning	What are some advantages of AJAX?	No page reloads, faster page renders, improved response times, client side rendering reduces network load.	2	\N	\N	1
...
21	19	learning	What is responsive web design?	Responsive Web Design is a set of practices that allows web pages to alter their layout and appearance to suit different screen widths and resolutions.	0	\N	\N	1
...
102	22	learning	In SQL, what convention applies to whether you can include lowercase or uppercase letters in a table or column name?	By convention, tables and columns are given names in snake_case. That is, words are separated by underscores and all letters are lower case.	9	\N	\N	1
...
48	13	learning	What is a web cookie?	A cookie is a name/value pair that stores information related to a particular user on their browser by a certain domain.	1	120	\N	1
...
80	31	learning	WTF?; DROP TABLE objectives; SELECT * FROM objectives 	JSX is a syntax that combines Javascript and HTML. For example, here is a line of valid JSX.\n\nconst simple = <h1>Hello, World!</h1>;\n\nWhen you're inside some markup, and want to include some Javascript, you need to enclose it in braces, thusly.\n\nconst simple = <h1>Hello, {planetNameVariable}!</h1>;\n\nAdjacent HTML tags must have a parent.	4	\N	\N	1
106	7	learning	What is Javascripts Event Loop?	Javascript's 'Event Loop' is a special feature of Javascript where the JS processor can stay alive after the main code has been parsed, and scheduled events will run processes in parallel.\n\nThis enables your code to stay on its feet, to react to user input or other inputs, while at the same time, managing the underlying OS's actions like reading files, downloading data from an API, gathering user input from a terminal, etc.	0	\N	\N	1
107	7	performance	How do you schedule a callback to be called (by the Event Loop) at some point in the future, after the current thread is finished?	setTimeout(callback, delay); // where callback is a function definition, and delay is a number of milliseconds minimum before the callback will be called. (It cannot be called at all, until after the current thread is done.)	1	\N	\N	1
111	7	learning	If you schedule a callback to be executed directly by the the Event Loop, can its return value be collected?	No.	2	\N	\N	1
108	7	learning	How can you configure a callback to be called at a regular interval?	setInterval(callback, timeBetweenCalls); // timeBetweenCalls is in milliseconds.	3	\N	\N	1
...
110	7	learning	What are some examples of tasks that would be better handled by a system that can run things in parallel?	Any time that a task may take some large amount of time, and that that time could potentially get in the way of more important tasks, consider scheduling the execution of that task on the Event Loop (i.e. in an asynchronous callback) instead of as a blocking call on the main thread.\n\nFor example: calling an API, reading a file, waiting for keystrokes of user input, waiting for mouse clicks on elements, etc.	5	\N	\N	1
112	9	learning	What is "callback hell"?	"Callback hell" is a problem with code, where the hierarchy of callbacks has gone so deep that debugging the code has become difficult.	0	\N	\N	1
116	11	learning	What is a web server?	A web server is a computer program that listens for TCP/IP connections and HTTP requests, processes the inputs that came within that request and sends an appropriate, customized response back to the computer that made the request.	1	\N	\N	1
117	11	learning	What is a Route?	A route is the combination of HTTP Request Verb (GET, POST, etc.) and a Path ("/contact", etc.)\n\nA web server chooses which code to build the specific response based on the route.	2	\N	\N	1
118	11	learning	What is Middleware?	In the context of an Express Web Server, middleware is code that will be executed for every route.\n\nExamples include parsing cookie or form data from the request headers, performing debugging logging, and performing authentication tasks.	4	\N	\N	1
12	14	learning	What is the difference between encryption and hashing?	Hashing is one way, so you can't recover the original text from the hash.	0	\N	\N	1
54	14	learning	What is HTTPS?	HTTP Secure is the encrypted version of HTTP. Using it means that the traffic between your computer and its destination cannot be read by any server or public wifi system in the delivery path.	3	\N	\N	1
...
119	39	learning	What is Object Oriented Programming?	OOP is a paradigm of programming where Classes are templates from which Objects are created. Classes are a mixture of variables and functions in one structure. Classes are the recipe, Objects are the individual instances of that recipe.	0	\N	\N	1
...
53	13	learning	What does it mean to say that HTTP is 'stateless'?	The HTTP protocol itself is unable to associate one request with another. Each request starts from scratch. Cookies are a payload within the headers of a request that enable a server to identify the browser session/user.	0	120	\N	1
43	12	learning	How often do you need to restart your web server?	Template files are reloaded every time they are rendered, so there is no need to restart your server if you have only changed your EJS template.\r\n\r\nIf, however, the server code itself changes, then the server will need to be restarted.\r\n\r\nNodemon is a helper program, available in the npm repository, that will run node programs and restart them when required.	6	120	\N	1
...
SELECT pg_catalog.setval('public.objectives_id_seq', 157, true);
